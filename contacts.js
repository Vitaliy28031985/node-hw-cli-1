...
async function readContacts () {
   try {
      const contactsContent = await fs.readFile(contactsPath, { encoding: 'utf8' });
...
try {
const text = JSON.stringify(dataToWrite);
...
async function listContacts() {
   ...
   if (contacts.lenght === 0) {
      console.log(`${yellow} There are no contacts.${reset}`);
      return; 
   }
...
if (!contacts) {
   console.log(`${red}There is no contact with the identifier ${contactId}!${reset}`);
   return;
}
console.log(`${bright}${green}Contact by id ${contactId}:${reset}${BGblue}`);
...
const contacts = await readContacts(newContact);
